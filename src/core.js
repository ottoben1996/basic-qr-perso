 ...
 * compte/club) ; si elle est absente, on retombe sur un tirage aléatoire comme
 * dans la première analyse publique du format.
 */
export async function buildPayload({ cardNumber, deviceId, constant, guid, iat }) {
  const g = (constant || guid || "").trim() || randomGuid();
  const ts = iat || Math.floor(Date.now() / 1000);
  const hash = await generateHash(cardNumber, g, ts, deviceId);
  return { payload: `GM2:${cardNumber}:${g}:${ts}:${hash}`, guid: g, iat: ts, hash };
}
